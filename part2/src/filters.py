...
class HardFilter:
    ...
    def _passes_filters(self, restaurant: Restaurant, parsed_query: ParsedQuery) -> bool:
        ...
        # CRITICAL FIX: For general "best X" queries, be very lenient with filters
        # Only apply strict filters for specific queries (restaurant name, explicit location, etc.)
        query_lower = parsed_query.raw_query.lower()
        is_general_best_query = any(keyword in query_lower for keyword in [
            'best', 'famous', 'popular', 'top', 'recommend', 'suggest', 'what', 'where to find',
            'scenic', 'view', 'views', 'oceanfront', 'beachfront', 'with'
        ])
        
        # For general queries, only filter by business status and must_not constraints
        # Skip cuisine/location filtering unless explicitly required
        if is_general_best_query:
            # Only apply must_not filters and business status
            ...
            # For general queries, skip strict cuisine/location filtering
            # Let scoring handle the relevance ranking instead
            logger.debug(f"✅ PASSED FILTER (general query): {restaurant.name}")
            return True
        
        # Continue with strict filtering for specific queries
        if restaurant.business_status and restaurant.business_status != "OPERATIONAL":
            return False
