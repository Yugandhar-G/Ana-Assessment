...
# Legacy WebSearch class kept for backward compatibility
# New code should use MultiSourceWebSearch from multi_source_search module
class WebSearch:
    """Legacy web search - use MultiSourceWebSearch instead."""
    
    def __init__(self, api_key: Optional[str] = None, search_engine_id: Optional[str] = None):
        # Import here to avoid circular dependency
        from .multi_source_search import MultiSourceWebSearch
        self._multi_source = MultiSourceWebSearch(
            google_api_key=api_key,
            google_search_engine_id=search_engine_id,
        )
    
    async def search(self, query: str, num_results: int = 5):
        """Perform web search using MultiSourceWebSearch."""
        from .multi_source_search import SearchSource
        results = await self._multi_source.search(
            query=query,
    ...
    async def close(self):
        ...
        await self._multi_source.close()
...
class AsyncGeminiClient:
    ...
    def __init__(
        ...
        # Initialize multi-source web search if enabled
        if enable_web_search:
            try:
                from .multi_source_search import MultiSourceWebSearch
                self.web_search = MultiSourceWebSearch(
...
class ChatCompletions:
    ...
    async def create(
        ...
        **kwargs
        ...
        # Note: Gemini's native web search tool is not available in the current SDK format
        # We use manual web search (Google Custom Search API) instead, which is already
        # integrated and working. The web search results are passed to Gemini in the prompt.
        # If native Gemini web search becomes available in the future, it can be enabled here.
        
        gemini_model = genai.GenerativeModel(
            model_name=model_name,
            system_instruction=system_instruction if system_instruction else None,
            safety_settings=self.client.safety_settings,
            # tools=None,  # Native web search not available in current SDK format
        )
        ...
        try:
            response = await asyncio.wait_for(
                ...
                timeout=120.0  # Increased from 90s to 120s for long responses
            )
    ...
    async def create_stream(
        ...
        **kwargs
        ...
        loop = asyncio.get_event_loop()
        try:
            # FIXED: Do NOT use list(response) as it blocks until the entire stream is finished
            # Instead, get the response iterator and iterate over it chunk by chunk
            response = await asyncio.wait_for(
                loop.run_in_executor(None, lambda: gemini_model.generate_content(
                    user_content,
                    generation_config=generation_config,
                    stream=True,
                )),
                timeout=60.0  # Increased timeout for initial response
            )
            
            # Iterate over the synchronous chunks in the executor to avoid blocking
            ...
            raise ConnectionError(
                "Gemini API streaming call timed out. "
                "The API might be slow or overloaded. Please try again."
            )
        except Exception as e:
            raise ConnectionError(
